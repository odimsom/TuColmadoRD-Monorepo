...
/// <summary>
/// Handles product deactivation command.
/// </summary>
public sealed class DeactivateProductCommandHandler : IRequestHandler<DeactivateProductCommand, OperationResult<ResultUnit, DomainError>>
{
    ...
    public async Task<OperationResult<ResultUnit, DomainError>> Handle(DeactivateProductCommand request, CancellationToken cancellationToken)
    {
        ...
        var payload = new ProductDeactivatedPayload(product.Id, tenantId, product.UpdatedAt);
        var outboxMessage = new OutboxMessage("ProductDeactivated", JsonSerializer.Serialize(payload));
