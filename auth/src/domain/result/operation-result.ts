export class OperationResult<T, E> {
  ...
  get value(): T {
    if (!this._ok) {
      throw new TypeError('Cannot read value of a failed OperationResult. Check isOk first.');
    }
  ...
  get error(): E {
    if (this._ok) {
      throw new TypeError('Cannot read error of a successful OperationResult. Check isOk first.');
    }
